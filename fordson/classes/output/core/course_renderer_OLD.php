...
class course_renderer extends \theme_boost\output\core\course_renderer {
            ...
            foreach ($acourseids as $courseids) {
                ...
                foreach ($courseids as $courseid) {
                    ...
                    if ($PAGE->theme->settings->coursetilestyle == 2) {
                        ...
                        $enrollbutton = get_string('access', 'theme_fordson');
                        $rowcontent .= '
                <div class="col-md-4">
                    ';
                        $rowcontent .= '
	...
	public function isAluno()
	{
		...
		if(!empty($USER->id))
		{
			...
			//has_capability('moodle/course:create', $coursecontext, $USER->id)
			
			if(has_capability('moodle/course:update', $coursecontext, $USER->id) || $this->isGuest() || $USER->id == 0)			
			{
				//Indica que é um professor, administrador ou convidado que está logado.
				return false;
			}else{
				//Indica que é um aluno.
				return true;
			}
		}
	...
	public function isAdmin()
	{
		...
		if(!empty($USER->id))
		{
			...
			if(has_capability('moodle/site:config', $coursecontext, $USER->id))			
			{
				//Indica que é um administrador que está logado.
				return true;
			}else{
				//Indica que não é um administrador que está logado.
				return false;
			}
		}
	...
	public function getCategoria()
	{
		...
		$auxArrayCurso = Array();
		$cont = 0;		
        $categoria_teste = $_GET["categoryid"];
       // $rcourseids = array_keys($courses);
		...
		$auxSql  = "SELECT  DISTINCT cc.id, cc.name, cc.description FROM mdl_course c ";
		$auxSql .= "INNER JOIN mdl_course_categories cc ON cc.id = c.category ";
		$auxSql .= "INNER JOIN mdl_context cx ON cx.instanceid = c.id ";
		$auxSql .= "INNER JOIN mdl_role_assignments r ON r.contextid = cx.id ";
		$auxSql .= "INNER JOIN mdl_user u ON u.id = r.userid ";
		...
		foreach ($auxArray as $categoria) 
		{
			


            $auxSql  = "SELECT DISTINCT c.id, c.fullname, c.summary FROM mdl_course c ";
            $auxSql .= "INNER JOIN mdl_course_categories cc ON cc.id = c.category ";
            $auxSql .= "INNER JOIN mdl_context cx ON cx.instanceid = c.id ";
            $auxSql .= "INNER JOIN mdl_role_assignments r ON r.contextid = cx.id ";
            $auxSql .= "INNER JOIN mdl_user u ON u.id = r.userid ";
            $auxSql .= "INNER JOIN mdl_user_enrolments ue on ue.userid = u.id and ue.enrolid in (select id from mdl_enrol where courseid = c.id) ";
            $auxSql .= "WHERE ue.status = 0 AND c.visible = 1 AND u.id = {$USER->id} AND cc.id = {$categoria->id} ORDER BY c.fullname";
            $auxArrayCurso = $DB->get_records_sql($auxSql);
									
			
			foreach ($auxArrayCurso as $curso) 
			{			
				//$auxSql  = "SELECT s.id, s.name, s.section, s.course FROM mdl_course_sections s ";
				//$auxSql .= "WHERE s.course = {$course->id} AND s.section > 0 AND s.name IS NOT NULL ORDER BY s.section";
				//$auxArraySecao = $DB->get_record_sql($auxSql);
					
                   // if($curso->id === $categoria->id){
			
					$imgurl = $OUTPUT->image_url('noimg', 'theme');
										
					if ($PAGE->theme->settings->coursetilestyle == 2) {
    
                        $enrollbutton = 'Acessar';
                        $rowcontent .= '
                <div class="col-md-3">
                    ';
                        $rowcontent .= '
                <div class="tilecontainer">
                        <figure class="coursestyle2">
                            ...
                            ';
                        if ($PAGE->theme->settings->titletooltip) {
                            $tooltiptext = 'data-toggle="tooltip" data-placement= "top" title="' . $curso->fullname . '"';
                        } else {
                            $tooltiptext = '';
                        }
                        $rowcontent .= html_writer::start_tag('div', array(
                        ...
                        $rowcontent .= '  
                                ...
                                <a ' . $tooltiptext . ' href="' . $CFG->wwwroot . '/course/view.php?id=' . $curso->id . '" class="coursestyle2url"></a>
                            </div>
                        </figure>
                </div>
                </div>
...
//}
    ...
    protected function coursecat_category(coursecat_helper $chelper, $coursecat, $depth) {
        ...
        $auxSql2  = "SELECT cc.id, cc.name, cc.description FROM mdl_course c ";
        $auxSql2 .= "INNER JOIN mdl_course_categories cc ON cc.id = c.category ";
        $auxSql2 .= "INNER JOIN mdl_context cx ON cx.instanceid = c.id ";
        $auxSql2 .= "INNER JOIN mdl_role_assignments r ON r.contextid = cx.id ";
        $auxSql2 .= "INNER JOIN mdl_user u ON u.id = r.userid ";
        $auxSql2 .= "WHERE cc.visible = 1 AND u.id = {$USER->id} AND cc.id = {$coursecat->id} GROUP BY cc.name ORDER BY cc.sortorder ASC";
        $auxArray2 = $DB->get_records_sql($auxSql2);

foreach ($auxArray2 as $categoria) {
 
        if($coursecat->id == $categoria->id){
...
//
     ...
     /*   $auxArray2 = Array();
        ...
        $auxSql2 .= "INNER JOIN mdl_role_assignments r ON r.contextid = cx.id ";
        $auxSql2 .= "INNER JOIN mdl_user u ON u.id = r.userid ";
        $auxSql2 .= "WHERE cc.visible = 1 AND u.id = {$USER->id} GROUP BY cc.name ORDER BY cc.sortorder ASC";
        $auxArray2 = $DB->get_records_sql($auxSql2);

        foreach ($auxArray2 as $categoria) {
            
          if($categoria->id == $coursecat->id){
       ...
       /* $classes[] = 'col-md-3 box-class';
        $content = '<div class="' . join(' ', $classes) . '" data-categoryid="' . $categoria->id . '" data-depth="' . $depth . '" data-showcourses="' . $chelper->get_show_courses() . '" data-type="' . self::COURSECAT_TYPE_CATEGORY . '">';
        $content .= '<div class="cat-icon">';
      ...
      //  $content .= '<div>';
      //  $content .= '<div class="info-enhanced">';
        $content .= '<span class="class-category">' . $categoria->name . '</span>';
    ...
    protected function coursecat_courses(coursecat_helper $chelper, $courses, $totalcount = null) {
		...
		// Valida se é admin ou aluno e exibe disciplinas.
		
		if($this->isAdmin() != 1){


		//$content .= $this->view_available_courses_aluno_prof($categoryid, $courses, $totalcount);	 
	    $content .= $this->getCategoria();
