...
//}
    ...
    protected function coursecat_category(coursecat_helper $chelper, $coursecat, $depth) {
		...
		/*** Inicio teste categoria ***/
		
	    $auxArray = Array();
		$auxArrayCurso = Array();
		

		$auxSql  = "SELECT cc.id, cc.name, cc.description FROM mdl_course c ";
		$auxSql .= "INNER JOIN mdl_course_categories cc ON cc.id = c.category ";
		$auxSql .= "INNER JOIN mdl_context cx ON cx.instanceid = c.id ";
		$auxSql .= "INNER JOIN mdl_role_assignments r ON r.contextid = cx.id ";
		$auxSql .= "INNER JOIN mdl_user u ON u.id = r.userid ";
		$auxSql .= "WHERE cc.visible = 1 AND u.id = {$USER->id} GROUP BY cc.name ORDER BY cc.sortorder ASC";
		$auxArray = $DB->get_records_sql($auxSql);
